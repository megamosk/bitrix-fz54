	...
	public function initiatePay(Payment $payment, Request $request = null)
	{
        ...
        foreach ($propertyCollection->getGroups() as $group)
        {
            foreach ($propertyCollection->getGroupProperties($group['ID']) as $property)
            {
                ...
                if( $p["CODE"] == "EMAIL")
                   $userContact = $property->getValue();
            }
        }
 
		//$userContact = (strlen($propertyCollection->getPhone())>0) ? $propertyCollection->getPhone() : $propertyCollection->getUserEmail();

		$arr = array();
		foreach ($basket as $key=>$basketItem) {
			$arr[$key]['quantity'] = $basketItem->getQuantity();
			$arr[$key]['price']['amount'] = $basketItem->getPrice();
			$arr[$key]['tax'] = 4;
			$arr[$key]['text'] = $basketItem->getField('NAME');
		}
		...
		$params = array(
			...
			'KASSA' => json_encode($ymReceipt, JSON_UNESCAPED_UNICODE)
		);
